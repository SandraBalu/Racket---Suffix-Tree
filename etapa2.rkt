...
(define (get-ch-words words ch)
    (filter (lambda (word) (and (not (null? word)) (starts-with? word ch))) words))
...
(define (ast-func suffixes)
  (cons (list (car (car suffixes))) (map rm-first-ch suffixes)))

(define (rm-first-ch word)
  (if (null? word)
      '()
      (cdr word)))
...
(define (cst-func suffixes)
  (define pref (longest-common-prefix-of-list suffixes))
  (append (list pref) (map (lambda (suff) (drop suff (length pref))) suffixes)))
...
(define text->st
  (lambda (label-func)
    (lambda (text)
      ...
      (define st (suffixes->st label-func (get-suffixes text-with-marker) alphabet))
        st)))
