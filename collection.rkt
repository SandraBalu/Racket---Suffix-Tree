...
(define-syntax-rule (collection-empty? flux) (stream-empty? flux))

(define-syntax-rule (collection-first flux)
  (if (stream-empty? flux)
      empty-stream
      (stream-first flux)))

(define-syntax-rule (collection-rest flux)
  (if (stream-empty? flux)
      empty-stream
  (stream-rest flux)))

(define (collection-filter predicate flux) (stream-filter predicate flux))

(define (collection-map flux s) (stream-map flux s))
